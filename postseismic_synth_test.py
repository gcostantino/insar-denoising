...
class DataGeneratorV2ForPostSeismic:
    ...
    def post_seismic_history(self):
        A = 1.0  # normalised amplitude
        tau = (self.Nt - 1) / 4.0  # relaxation time as a function of Nt → tau(Nt) = (Nt-1)/4 for example

        # ── Discrete time series ──────────────────────────────────────────────────────
        tspace = np.arange(self.Nt, dtype=float) * self.parameters['dt']
        u = A * np.log1p(tspace / tau)  # u(t) = A·log(1 + t/τ),  u[0] = 0 exactly
        #plt.plot(tspace, u)
        #plt.show()
        #t_cont = np.linspace(0, N - 1, 300)
        #u_cont = A * np.log1p(t_cont / tau)
    ...
    def model_fault_deformation(self):
        ...
        temporal_slip_evolution = self.post_seismic_history()
    ...
    def generate_batch_data(self, return_components=False, return_temporal_ev=False):
        """
...
def post_seismic_relaxation(params, seed):
    params['n_faults'] = 1
    ...
    generator = DataGeneratorV2ForPostSeismic(params, dtype=float, verbose=False, seed=seed)
    print('using seed:', generator.seed)
    batch_x, batch_y, batch_topo = generator.generate_batch_data(return_components=False)
    return batch_x, batch_y, batch_topo

def plot_results_post(data_examples, truth_examples, pred_examples):
    ...
    # ---------- subgroup 1: rows 1–3 ----------
    #print(data_examples[0].min(), data_examples[0].max())
    #print(truth_examples[0].min(), truth_examples[0].max())
    norm1_data = Normalize(vmin=data_examples[0].min(), vmax=data_examples[0].max())
    norm1_truth = TwoSlopeNorm(vmin=truth_examples[0].min(), vcenter=0, vmax=truth_examples[0].max())
    plot_subgroup(example_idx=0, row_start=1, title='Post-seismic relaxation test',
    ...
    plot_subgroup(example_idx=1, row_start=5, title='Fast rupture',
    ...
    plot_subgroup(example_idx=2, row_start=9, title='Complex fault trace',
...
def plot_results_post_residuals(data_examples, truth_examples, pred_examples):
    ...
    plot_subgroup(example_idx=0, row_start=1, title=rf'$\mathbf{{Post-seismic\ relaxation\ test\ (SNR={snr_list[0]:.2f})}}$',
    ...
    # plt.show()
    plt.savefig('/Users/giuseppe/Documents/DATA/InSAR/insar-denoising/figures/supp/synthetic_postseismic.pdf',
                bbox_inches='tight')

def plot_transects_synth_post(x, y, pred):
    ...
    ax_right_top.plot(dist_pred, profile_pred, color='C3', lw=2., label='Denoised')
    ...
    # fig.tight_layout()
    # plt.show()
    plt.savefig('/Users/giuseppe/Documents/DATA/InSAR/insar-denoising/figures/supp/postseismic_transects.pdf',
                bbox_inches='tight')
...
if __name__ == '__main__':
    ...
    post_x, post_y, topo_post = post_seismic_relaxation(params, seed=903693795)  # 616459241  # 3263276907  # 6394521764281  # 3985047843

    post_x = np.astype(post_x, np.float32)
    post_y = np.astype(post_y, np.float32)
    topo_post = np.astype(topo_post, np.float32)

    # normalize topography
    topo_post = normalize_topo(topo_post)

    pred = predict(denoiser_configuration, post_x, post_y, topo_post)

    post_x = post_x.squeeze()
    post_y = post_y.squeeze()
    topo_post = topo_post.squeeze()
    pred = pred.squeeze()
    ...
    plot_results_post_residuals(post_x, post_y, pred)

    post_x = post_x.squeeze()
    post_y = post_y.squeeze()
    pred = pred.squeeze()

    plot_transects_synth_post(post_x, post_y, pred)
